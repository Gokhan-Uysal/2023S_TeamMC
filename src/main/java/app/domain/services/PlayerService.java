...
public class PlayerService {
    ...
    public void createPlayer(ArrayList<String> names) {
        for (int i = 0; i < names.size(); i++) {
            Player newPlayer = new Player(i + 1, names.get(i));
            _players.add(newPlayer);
        }
    }

    public Player getPlayer(int playerId) throws NullPointerException {
        for (Player player : _players) {
            if (player.getId().equals(playerId)) {
                return player;
            }
        }
        throw new NullPointerException(String.format("User nor found by id: %d", playerId));
    }
    ...
    // public boolean attack(int attackingPlayerId, int attackedPlayerId, int
    // attackerTerritoryId,
    // int attackedTerritoryId) {

    // Territory attackerTerritory =
    // getPlayer(attackingPlayerId).getTerritory(attackerTerritoryId);
    // Territory attackedTerritory =
    // getPlayer(attackedPlayerId).getTerritory(attackedTerritoryId);
    ...
    // if (attackerTerritory.getTerritoryArmy().getArmyAmount(ArmyUnitType.Infantry)
    // > 0) {
    // attackerTerritory.getTerritoryArmy().transferArmyUnits(attackedTerritory.getTerritoryArmy(),
    // ArmyUnitType.Infantry, 1);
    // } else if
    // (attackerTerritory.getTerritoryArmy().getArmyAmount(ArmyUnitType.Chivalry) >
    // 0) {
    // attackerTerritory.getTerritoryArmy().transferArmyUnits(attackedTerritory.getTerritoryArmy(),
    // ArmyUnitType.Chivalry, 1);
    // } else if
    // (attackedTerritory.getTerritoryArmy().getArmyAmount(ArmyUnitType.Artillery) >
    // 0) {
    // attackerTerritory.getTerritoryArmy().transferArmyUnits(attackedTerritory.getTerritoryArmy(),
    ...
    // // private ArrayList<Territory> getTerritoriesFromTerritoryCards(int
    // playerId) {
    ...
    // public boolean tradeArmyCards(int infantryCardAmount, int cavalryCardAmount,
    // int artilleryCardAmount, int playerId,
    // int territoryId) {

    // Player tradingPlayer = getPlayer(playerId);

    // if (!this.checkIfArmyCardTradable(infantryCardAmount, cavalryCardAmount,
    // artilleryCardAmount, playerId)) {
    // if (infantryCardAmount == 3 && cavalryCardAmount == 0 && artilleryCardAmount
    // == 0) {
    // tradingPlayer.getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Chivalry,
    // 1);
    // } else if (infantryCardAmount == 2 && cavalryCardAmount == 1 &&
    // artilleryCardAmount == 0) {
    // getPlayer(playerId).getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Chivalry,
    // 2);
    // } else if (infantryCardAmount == 2 && cavalryCardAmount == 0 &&
    // artilleryCardAmount == 1) {
    // getPlayer(playerId).getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Artillery,
    // 2);
    // } else if (infantryCardAmount == 1 && cavalryCardAmount == 2 &&
    // artilleryCardAmount == 0) {
    // getPlayer(playerId).getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Chivalry,
    // 1);
    // getPlayer(playerId).getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Artillery,
    // 1);
    // } else if (infantryCardAmount == 0 && cavalryCardAmount == 2 &&
    // artilleryCardAmount == 1) {
    // getPlayer(playerId).getTerritory(territoryId).getTerritoryArmy().addArmyUnits(ArmyUnitType.Artillery,
    // 3);
    // }
    ...
    // tradingPlayer.getPlayerDecks().drawCard(ArmyCardType.Infantry);
    // }
    // for (int i = 0; i < cavalryCardAmount; i++) {
    // tradingPlayer.getPlayerDecks().drawCard(ArmyCardType.Cavalry);
    // }
    // for (int i = 0; i < artilleryCardAmount; i++) {
    // tradingPlayer.getPlayerDecks().drawCard(ArmyCardType.Artillery);
    // }
    ...
    // public boolean checkIfArmyCardTradable(int infantryCardAmount, int
    // cavalryCardAmount, int artilleryCardAmount,
    // int playerId) {

    // Player tradingPlayer = getPlayer(playerId);

    // return tradingPlayer.getPlayerDecks().findCardAmount(CardType.Infantry) >=
    // infantryCardAmount &&
    // tradingPlayer.getPlayerDecks().findCardAmount(CardType.Cavalry) >=
    // cavalryCardAmount &&
    // tradingPlayer.getPlayerDecks().findCardAmount(CardType.Artillery) >=
    // artilleryCardAmount;
    // }

    // private boolean doesPlayerOwnTerritory(int playerId, int territoryId) {
    // return
    // getPlayer(playerId).getTerritoryList().contains(_mapService.findTerritory(territoryId));
    // }
    ...
    private int getPlayerInitialArmyAmount() {
        return 40 - (getPlayerCount() - 2) * 5;
    }
